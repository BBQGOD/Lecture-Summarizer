...
def run_thread_pool_sub(target, client, data, params, max_work_count: int):
    with ThreadPoolExecutor(max_workers=max_work_count) as t:
        futures = [t.submit(target, client, i, data[i], params)
                    for i in range(len(data))]
        for future in as_completed(futures):
            yield future.result()


def _post_request(client, data, params: dict):
    ret_list = []
    for sample in tqdm(data, desc="Requesting OpenAI API"):
        # print(sample)
        response = client.chat.completions.create(
            messages=sample,
            **params
        )
        ret_list.append(response.choices[0].message.content)
    return ret_list
...
def thread_function(client, idx: int, chk: List[Any], params: dict):
    lp = _post_request(client, chk, params)
    return idx, lp
