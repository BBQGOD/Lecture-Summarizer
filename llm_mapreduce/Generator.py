...
from llm_mapreduce.utils import print_intermediate_output,  run_thread_pool_sub, split_list_of_docs, thread_function
...
class Generator:
    def __init__(
        ...
        self.tokenizer = tiktoken.encoding_for_model(config['llm']['name_or_path'])
        self.config = config
    ...
    def get_batch_reply(self, batch):
        ...
        result_map = {}
        res = []
        for i, result_list in tqdm(run_thread_pool_sub(
            thread_function, self.client_gpt, chunk_req, self.gen_args, min(
